    ...
    >
      ...
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        {restaurant.trending && (
          <motion.div
            ...
            className="absolute top-3 left-3 flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-primary/90 backdrop-blur-sm text-white text-xs font-medium"
          >
      ...
      <div className="p-4">
        ...
        <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground mb-3">
          ...
          <span className="text-amber-600 font-medium">{priceRange}</span>
        </div>
        ...
        {restaurant.dietaryOptions.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-3">
            {restaurant.dietaryOptions.slice(0, 3).map((option) => (
              <span
                key={option}
                className="px-2 py-0.5 rounded-full bg-emerald/10 text-emerald text-xs font-medium"
              >
                {option}
              </span>
            ))}
        ...
        {/* Spice Level */}
        {restaurant.spiceLevel > 0 && (
          <div className="flex items-center gap-1 mt-3">
            {Array.from({ length: restaurant.spiceLevel }).map((_, i) => (
              <span key={i} className="text-sm">🌶️</span>
            ))}
            <span className="text-xs text-muted-foreground ml-1">
              {restaurant.spiceLevel === 1 && 'Mild'}
              {restaurant.spiceLevel === 2 && 'Medium'}
              {restaurant.spiceLevel === 3 && 'Spicy'}
            </span>
          </div>
        )}
      </div>
